...
public class DependentService : IDependentService
{
    ...
    public async Task<DependentResponseDto> Add(DependentRequestDto request)
    {
        ...
        dependents.DateOfBirt = request.DateOfBirth;
    ...
    {
        var dependents = await _dependentRepository.FindBy(x => x.DependId == id).FirstOrDefaultAsync();
    ...
    {
        var dependents = await _dependentRepository.FindByAsNoTracking(x => x.DependId == id).FirstOrDefaultAsync();
        var response = _mapper.Map<DependentResponseDto>(dependents);
        return response;
    }
